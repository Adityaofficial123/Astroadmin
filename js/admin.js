...
function fetchAnnouncements() {
    ...
    db.ref('announcements').on('value', (snap) => {
        ...
        const countEl = document.getElementById('dashAnnouncementCount');
        if (countEl) countEl.textContent = Object.keys(announcementsData).length;
        renderAnnouncements();
    });
}

function normalizeAnnouncementStatus(raw) {
    const value = String(raw || '').trim().toLowerCase();
    return value === 'draft' ? 'draft' : 'published';
}

function getAnnouncementTimestamp(item) {
    return toTs(item?.publishDate) || toTs(item?.publishedAt) || toTs(item?.updatedAt) || toTs(item?.createdAt);
}
...
function renderAnnouncements() {
    ...
    entries.sort((a, b) => {
        const aPinned = a[1]?.isPinned ? 1 : 0;
        const bPinned = b[1]?.isPinned ? 1 : 0;
        if (aPinned !== bPinned) return bPinned - aPinned;
        return getAnnouncementTimestamp(b[1]) - getAnnouncementTimestamp(a[1]);
    });

    if (!entries.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="p-5 text-gray-400">No announcements posted yet.</td></tr>';
        return;
    }

    tbody.innerHTML = entries.map(([id, item]) => {
        const status = normalizeAnnouncementStatus(item?.status);
        const statusClass = status === 'published' ? 'text-green-400' : 'text-yellow-300';
        const when = fmtDate(item?.publishDate || item?.publishedAt || item?.updatedAt || item?.createdAt);
        const tag = String(item?.tag || '').trim();
        const bodyPreview = String(item?.body || '').trim().slice(0, 120);

        return `
            <tr class="table-row">
                <td class="p-4">
                    <div class="font-semibold text-white flex items-center gap-2">
                        ${item?.isPinned ? '<span class="text-cyan-300 text-xs uppercase tracking-wide">Pinned</span>' : ''}
                        <span>${esc(item?.title || 'Untitled Announcement')}</span>
                    </div>
                    <div class="text-xs text-gray-400 mt-1">${tag ? `[${esc(tag)}] ` : ''}${esc(bodyPreview)}${String(item?.body || '').length > 120 ? '...' : ''}</div>
                </td>
                <td class="p-4 uppercase text-xs tracking-wide ${statusClass}">${status}</td>
                <td class="p-4 text-gray-300">${esc(when)}</td>
                <td class="p-4 text-right">
                    <button type="button" onclick="editAnnouncement('${id}')" class="text-xs text-blue-400 uppercase hover:underline mr-3">Edit</button>
                    <button type="button" onclick="deleteAnnouncement('${id}')" class="text-xs text-red-400 uppercase hover:underline">Delete</button>
                </td>
            </tr>
        `;
    }).join('');
...
window.resetAnnouncementForm = () => {
    ...
    const idEl = document.getElementById('aId');
    ...
    const bodyEl = document.getElementById('aBody');
    const linkLabelEl = document.getElementById('aLinkLabel');
    const linkUrlEl = document.getElementById('aLinkUrl');
    const statusEl = document.getElementById('aStatus');
    const pinnedEl = document.getElementById('aPinned');
...
window.editAnnouncement = (id) => {
    ...
    if (dateEl) dateEl.value = toInputDate(item.publishDate || item.publishedAt || item.updatedAt || item.createdAt);
    if (tagEl) tagEl.value = item.tag || '';
    if (bodyEl) bodyEl.value = item.body || '';
    if (linkLabelEl) linkLabelEl.value = item.linkLabel || '';
    if (linkUrlEl) linkUrlEl.value = item.linkUrl || '';
    if (statusEl) statusEl.value = normalizeAnnouncementStatus(item.status);
    if (pinnedEl) pinnedEl.checked = !!item.isPinned;
    if (btn) btn.innerText = 'Update Announcement';
...
document.getElementById('announcementForm')?.addEventListener('submit', async (e) => {
    ...
    const id = String(document.getElementById('aId')?.value || '').trim();
    const title = String(document.getElementById('aTitle')?.value || '').trim();
    const publishDate = String(document.getElementById('aDate')?.value || '').trim();
    const tag = String(document.getElementById('aTag')?.value || '').trim();
    const body = String(document.getElementById('aBody')?.value || '').trim();
    const linkLabel = String(document.getElementById('aLinkLabel')?.value || '').trim();
    const linkUrlRaw = String(document.getElementById('aLinkUrl')?.value || '').trim();
    const status = normalizeAnnouncementStatus(document.getElementById('aStatus')?.value);
    ...
    if (!title || !body || !publishDate) {
        ...
        if (btn) {
            btn.disabled = false;
            btn.innerText = id ? 'Update Announcement' : 'Publish Announcement';
        }
        return;
    }

    const old = announcementsData?.[id] || {};
    const payload = {
        title,
        tag,
        body,
        linkLabel,
        linkUrl: /^https?:\/\//i.test(linkUrlRaw) ? linkUrlRaw : '',
        status,
        isPinned: isPinned && status === 'published',
        publishDate,
        createdAt: old.createdAt || Date.now(),
        updatedAt: Date.now(),
        publishedAt: status === 'published' ? (old.publishedAt || Date.now()) : 0
    };

    try {
        const targetRef = id ? db.ref('announcements/' + id) : db.ref('announcements').push();
        const key = id || targetRef.key;
        const updates = {
            ['announcements/' + key]: payload
        };

        if (payload.isPinned) {
            Object.entries(announcementsData).forEach(([existingKey, existing]) => {
                if (existingKey !== key && existing?.isPinned) {
                    updates['announcements/' + existingKey + '/isPinned'] = false;
                }
            });
        }
        ...
        toggleForm('announcementFormContainer');
    } catch (err) {
